...
*Source:* `Codility/Lessons/Lesson5/Fish.java`
...
public class Fish {
    public int solution(int[] A, int[] B) {
        ...
        for (int i = 0; i < A.length; i++) {
            ...
            if (direction == 0) {
                while (!right.isEmpty() && fishSize > right.peek()) {
                    right.pop();
                }
                if (right.isEmpty()) {
                    ans++;
    ...
    public static void main(String[] args) {
        System.out.println(new Fish().solution(new int[] { 4, 3, 6, 2, 1, 5 }, new int[] { 0, 1, 1, 0, 0, 0 }));
