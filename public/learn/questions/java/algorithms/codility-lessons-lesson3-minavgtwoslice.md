# Min Avg Two Slice — Lesson 3

**Origin:** Codility &nbsp;|&nbsp; **Topics:** Prefix Sums

*Source:* `Codility/Lessons/Lesson3/MinAvgTwoSlice.java`
...
A (notice that the slice contains at least two elements). The average of a slice (P, Q) is the sum of A[P] + A[P + 1] + ... + A[Q] divided by the
...
public class MinAvgTwoSlice {
    public int solution(int[] A) {
        ...
        int ans = -1;
        // the best slice must be length = 2 or 3.
        for (int i = 0; i < A.length - 2; i++) {
            long sum = Math.min((A[i] + A[i + 1]) * 3, (A[i] + A[i + 1] + A[i + 2]) * 2);
            if (sum < min) {
                min = sum;
                ans = i;
            }
    ...
    public int solutionPrefix(int[] A) {
